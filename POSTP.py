...
def POSTP(MZ,NZ,LCG,NSTP,IDLT,JHR0,NHR,NSIZE,BB,IPS,CRHO,
    ...
    INTEGER     IDLT(NSTP)              予熱開始後、各ステップにおいて直後室温湿度を=0：未知数としない、=1：未知数とする
    INTEGER     JHR0                    予熱を開始した時刻
    INTEGER     NHR                     1日のステップ数
    INTEGER     NSIZE                   方程式の数（未知数の数）
    REAL*8      BB(NSIZE)               連立方程式の解（右辺ベクトル）
    INTEGER     IPS(0:NSTP)             予熱開始後、各時刻ステップの室温湿度が、未知ベクトルの何次元目から始まるか（=IPS+1次元目から
    REAL        CRHO                    空気の容積比熱（潜熱の場合は密度に蒸発潜熱を掛けたもの）
    REAL        VOA(MZ,0:1,NHR)         各ゾーンの外気量（体積流量、0以上）
    REAL        OATX(MZ,NHR)            導入外界温湿度（外調機考慮、基準温湿度からの偏差ではない）
    REAL        REFWD                   基準温湿度
    INTEGER     ISL                     =1:顕熱、=2:潜熱
    REAL        RMSET(NZ)               設定温湿度（基準温湿度からの偏差ではない）
    INTEGER     NTRM                    蓄熱応答係数の項数
    INTEGER     LSTP(NTRM)              蓄熱応答係数（瞬時分を除く）へのポインタ
    INTEGER     LSTQ(NTRM)              蓄熱負荷（瞬時分を除く）へのポインタ
    ...
    INTEGER     LMODE(MZ,0:1,NHR)       各ゾーンの負荷状態モード（-2：暖房過負過,-1：暖房軽負荷,0：無負荷,1：冷房軽負荷,2：冷房過負荷,9：停止）
    REAL        AN(MZ,0:1,NHR)          各ゾーンの装置除去熱量
    REAL        RM(MZ,0:1,NHR)          各ゾーンの室温湿度（基準温湿度からの偏差）(引用されるのは予熱開始直前のデータのみ)
    REAL        RN(MZ,0:1,NHR)          各ゾーンの室除去熱量
    REAL        AMRT(MZ,0:1,NHR)        MRT
    REAL*8      WK(NTRM)
    ...
    INTEGER     IREPS   
    INTEGER     IREPE   
    INTEGER     IREP2                   現在の時刻ステップの直前か直後か=0：直前あるいは二等辺三角、=1：直後
    INTEGER     JHR2                    時刻
    REAL        RMWK
    INTEGER     J
    INTEGER     LSTPWK
