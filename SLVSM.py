...
def SLVSM(NZ,IOPT,EXCAP,SPCAP,NAZ,VFLOW,P0,CRHO,VOA,
    ...
    REAL        VFLOW(NAZ,NZ)   ! I   第1添字目のゾーンから第2添字目のゾーンへの流入風量（体積流量、0以上、対角項は0となっていなければならない）
    ...
    REAL        RMMN(NZ)        ! I   各スペースの設定温湿度下限（RMMX,RMMNは基準温湿度からの偏差ではない）
    REAL        REFWD           ! I   基準温湿度
    ...
    INTEGER     IREP            ! I   現在の時刻ステップの直前か直後か =0：直前あるいは二等辺三角、=1：直後
    INTEGER     LCG(NZ)         ! I   XMQ配列のSPACデータへのポインタ（L）
    INTEGER     LSZSPC(0:4)     ! I   XMQ配列のうち、(0):SPAC, (1):OWAL, (2):IWAL, (3):WNDW, (4):INFL の変数の数
    INTEGER     LMODE(NZ)       ! O   各ゾーンの負荷状態モード（-2：暖房過負荷,-1：暖房軽負荷,0：無負荷,1：冷房軽負荷,2：冷房過負荷,9：停止）
